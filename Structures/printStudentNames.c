...
createStudent(), append() as well as any other functions you added, 
such as copyStr() for example.

Your new task is to write a function printStudents() 
which takes as input a pointer that holds the address
of the start of a linked list of students. Your function should
then print this list of students to the screen as specified in the 
example below. Your function should not return anything.*/
...
void printStudents(struct student *start){
    //why not start->next !=NULL --> skip the last one
    while (start != NULL){
        printf("%p is %d years old.\n", start->name, start->age);
    }
